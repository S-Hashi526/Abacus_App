class AttendancesController < ApplicationController
  ...
  before_action :set_user_user_id, only: [:update_overwork_notice, :update_onemonth_approval, :log_page]
  ...
  before_action :admin_or_correct_user, only: [:edit_one_month, :edit_attendance_change, :edit_overwork_notice, :log_page]
  ...
  def update_month_request
    ...
    if month_request_params[:superior_month_notice_confirmation].present?
      ...
      flash[:success] = "#{@user.name}の1か月分の申請を実施しました。"
    else
      flash[:danger] = "所属長を選択してください。"
    end
    redirect_to user_url(@user)
  ...
  private
    ...
    def month_request_params
      params.requiew(:attendance).permit(:superior_month_notice_confirmation, :one_month_approval_status)
    end
